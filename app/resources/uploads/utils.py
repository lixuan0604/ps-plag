...
from common.models.user import InvertTable, PublicFile, PublicSent, IndiaOrder
import pandas as pd
...
def process_docx(file, skip_short=True):
    ...
    for para in paragraphs:
        ...
        paragraph_index += 1
    # print(doc_sentences)
    # file_content = {str(file): doc_sentences}
    # file_content = (str(file), doc_sentences)
    ...
    #     bad_file_size.append(os.path.getsize(file))
    #     print('*'*50)


def doc2docx(file):
    doc2docxF = os.path.join(Path(__file__).resolve().parent.parent.parent.parent, 'doc2docxF')
...
def get_word_dict():
    word_info = InvertTable.query.all()
    if len(word_info) == 0:
        word_dict = {}
    else:
...
def content2table(filename, content):
    # sent_table = PublicSent.query.all()
    # if len(sent_table) == 0:
    #     sent_table_set = set()
    # else:
    #     sent_table_set = set(info.sent for info in sent_table)
    WORD_DICT = get_word_dict()
    ...
    for sent_dict in content:
        ...
        if zhPattern.search(sent):
            continue
        # if sent.lower() in sent_table_set:
        #     continue
        sen_lem = lemmatize(sent.lower())
        p_sent = PublicSent(sent=sent.strip(), file_path=filename, p_i=sent_id, len_sent=len(sen_lem))
        db.session.add(p_sent)
        db.session.flush()
        index_id = p_sent.id
        for word in sen_lem:
            if word in stop_words or re.search('[0-9]+', word) or word.startswith("'") or word.endswith("'") or word.startswith("-") or word.endswith("-") or len(word) < 3:
                continue
            if not WORD_DICT.get(word):
                word_invert = InvertTable(word=word)
                db.session.add(word_invert)
                db.session.flush()
                WORD_DICT[word] = {}
                WORD_DICT[word]['id'] = word_invert.id
                WORD_DICT[word]['word_sent_list'] = set()
            WORD_DICT[word]['word_sent_list'].add(index_id)
            # WORD_DICT[word].setdefault('word_sent_list', set()).add(index_id)
    db.session.commit()
    for k, v in WORD_DICT.items():
        word = k
        id = v['id']
        word_sent_list = str(v['word_sent_list'])
        InvertTable.query.filter_by(id=id).update({'word_sent_list': word_sent_list})
...
def build_invert_table():
    tmp = PublicFile.query.all()
    table_file_set = set(info.file_path for info in tmp)
    public_file = set(get_all_file())
    file_difference = list(public_file.difference(table_file_set))

    for file in tqdm(file_difference):
        try:
            filename, content = process_docx(file)
            content2table(filename, content)
        except Exception as e:
            print(str(e))
            tmp_file = PublicFile(status=0, file_path=str(file))
            db.session.add(tmp_file)
...
def plag_word(file, exclude_order=True, min_word_count=4):
    try:
        ...
        for sent_dict in content:
            ...
            for word in sen_lem:
                if word in stop_words:
                    continue
                word_info = InvertTable.query.filter_by(word=word).first()
                if not word_info:
                    continue
            ...
            for id_sent in sent_count_list:
                sent_info = PublicSent.query.filter_by(id=id_sent).first()
...
def plag(id, fid, file, exclude_order=True, update_invert=True, min_word_count=4):
    if update_invert:
        build_invert_table_()
    PLAG_DETALI = os.path.join(Path(__file__).resolve().parent.parent.parent.parent, 'plag_detail')
    ...
    if list_detail:
        ...
        IndiaOrder.query.filter_by(id=id).update({'score': average_rouge_l, 'status': 1, 'end_time': time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), 'detail_path': os.path.join(PLAG_DETALI, str(fid) + '.json')})
    else:
        IndiaOrder.query.filter_by(id=id).update({'score': -1, 'status': -1, 'end_time': time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())})
    db.session.commit()


if __name__ == '__main__':
    # process_docx('../../../ankita_liuxuejiang_23_edinburgh_llm_29nov.docx')
    # files = get_all_file()
    # for file in files:
    #     process_docx(file)
    # print(len(bad_file))
    # print(sum(bad_file_size)/len(bad_file))
    # build_invert_table()
    # file = '/home/mary/wenshu/public/chenyy/chenyy_12/india_os/ushana_9ws_chenyy_12_7dec_1225.docx'
    # print('/'.join(file.split('/')[:7]))
    # Pattern = re.compile(u'[0-9\_\'\"\+\·]+')
    # print(Pattern.search(""))
    # print(unicode_to_ascii("learning plan: read books related to (the theory of the major） before enrolling in the program, to become familiar with what needs to be learned as soon as possible, and to make up for the lack of my own previous learning background"))
    # print(del_symbol("learning 10plan: read books related to (the theory of the major） before enrolling in the program, to become familiar with what needs to be learned as soon as possible, and to make up for the lack of my own previous learning background"))
    # # print(re.search('[0-9]+', ''))
    # print(unicode_to_ascii('​what law​'))
    # print('​what law​ law​ ﬁfteenth diﬀusion competence‎')
    # print(sentence_bleu([word_tokenize('The hypothesis contains 0 counts of 3-gram overlaps.')], word_tokenize('The hypothesis contains 0 counts of 3-gram overlaps.'), weights=(1, 0, 0, 0)))
    path = '/home/mary/wenshu/public/test/test_001/india_os/硕士个人陈述调查问卷1012批注.doc'
    os.system(f"libreoffice --headless --convert-to docx {path}")